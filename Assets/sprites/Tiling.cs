...
public class Tiling : MonoBehaviour {
	...
	void Start () {
		...
		spriteWidth = sRenderer.sprite.bounds.size.y;
	}
	...
	void Update () {
		...
		if (hasALeftBuddy == false || hasARightBuddy == false) {
			...
			float edgeVisiblePositionRight = (myTransform.position.y + spriteWidth/2) - camHorizontalExtend;
	...
	void MakeNewBuddy (int rightOrLeft) {
		...
		Vector3 newPosition = new Vector3 (myTransform.position.y + spriteWidth * rightOrLeft, myTransform.position.y, myTransform.position.z);
		// instantating our new body and storing him in a variable
		Transform newBuddy = Instantiate (myTransform, newPosition, myTransform.rotation) as Transform;

		// if not tilable let's reverse the x size og our object to get rid of ugly seams
		if (reverseScale == true) {
			newBuddy.localScale = new Vector3 (newBuddy.localScale.y*-1, newBuddy.localScale.y, newBuddy.localScale.z);
		}
